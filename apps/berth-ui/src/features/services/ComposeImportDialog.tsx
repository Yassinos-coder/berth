...
export function ComposeImportDialog() {
  ...
  return <Dialog open={open} onOpenChange={setOpen}><DialogTrigger asChild><Button variant="outline"><FileUp className="size-4" /> Import Compose</Button></DialogTrigger><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle>Import Docker Compose</DialogTitle><DialogDescription>Image-based services, environment variables, commands, and exposed ports are imported.</DialogDescription></DialogHeader>
    ...
    <Button disabled={!serverId || !document.trim() || mutation.isPending} onClick={() => mutation.mutate()}>{mutation.isPending ? <Loader2 className="size-4 animate-spin" /> : <FileUp className="size-4" />} Import services</Button></div>
